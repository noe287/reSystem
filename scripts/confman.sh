...
if [ $# -ne 2 ];then
        ...
        echo "Example use: confman.sh CONFIG_ATLANTIS_BUILDSYS_REVISION 4773bed2e2b05bbbe1cfb8888e4cd72718def04e"
        exit
fi
...
for target in $profiles; do
	...
	target_config=`find . -name "$target-release.config"`

	if [ "$target_config" != "" ]; then
		echo "found the config file for $target"
		#config_content=`cat $target_config`
		#echo "cont: $config_content"
		result=`grep $target_conf_param $target_config -r|awk -F "=" '{print $2}'|awk -F "\"" '{print $2}'`
		if [ "$result" != "" ]; then
