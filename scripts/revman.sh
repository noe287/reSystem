...
if [ $# -ne 2 ];then
        ...
        echo "Example use: revman.sh automesh 4773bed2e2b05bbbe1cfb8888e4cd72718def04e"
        exit
fi
...
for target in $profiles; do
	...
	if [ "$target_config" != "" ]; then
		...
		result=`grep $target_package"_CHECKOUT_REVISION" $target_config -r|awk -F "=" '{print $2}'|awk -F "\"" '{print $2}'`
		if [ "$result" != "" ]; then
